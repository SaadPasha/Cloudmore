...
@allure.title("Delete User - Invalid and Null Usernames")
@allure.severity(allure.severity_level.CRITICAL)
@pytest.mark.parametrize("username, expected", [(api_utils.gen_rand_str(5), 404),
                                                (api_utils.gen_rand_int(10000, 99999), 404),
                                                (" ", 404)])
def test_delete_user_details_invalid_and_null_username(username, expected):
    """
    ...
    with allure.step(STEP_1.format(username, username)):
        assert api_utils.delete_user(u_name=username).status_code == expected
